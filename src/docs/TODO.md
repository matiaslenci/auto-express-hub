 ## Landing

- [ ] Modificar datos no correctos del Hero como la cantidad de agencias y vehículos
- [ ] Arreglar el badge del hero en vista mobile
- [ ] Definir precios y cantidad de publicaciones

 ## Dashboard

- [ ] Revisar detalles de agregar vehículo
- [ ] Implementar la funcionalidad de agregar imagenes con el back
- [ ] Crear seed en el back para demo y eliminar los datos hardcodeados
- [ ] Agregar al sidebar la opción de cambiar plan (siempre que sea menor al plan actual motivar a subirlo)
- [ ] Boton para compartir catalogo

## Perfil

- [ ] Poder editar la foto de perfil y portada, agregar desde el backend
- [ ] Cambiar plan actual

## Catalogo

- [ ] Hacer tests
- [ ] Modificar los tipos de combustible
- [ ] Arreglar ui de filtros de precio y año
- [ ] Eliminar tantos detalles naranjas
- [ ] Arreglar badge de tipo de vehiculo(suv, sedan, etc)

## Cargar vehiculo

- [ ] Cambiar entre precio ARS, USD o Sin precio(modificar backend)